...
public class HexaGrid {
	...
	//Author : ?	
	//Modifié le 22/03/2023 par L3C1, Yuting HUANG
	public void createRandomRectGrid(int w,int h){
		...
		this.h = h;
		for (int j=0;j<h;j++){
			Debug.Log("Boucle for 1");
			for (int i=0;i<w;i++){
				Debug.Log("Boucle for 2");
				Debug.Log("--ADD--");
				hexaList.Add(new Hexa((((int)Random.Range(0, 8)) <= 1) ? HexaType.WALL : HexaType.GROUND, i, j));

				Debug.Log("Fin Boucle for 2");
				}
			Debug.Log(" Fin Boucle for 1");
			}
		Debug.Log("Fin méthode");
	}
	
	//Auteur : Yuting HUANG
	//Date : 22/03/2023
	//Objectif : Amélioration des terrains aléatoires
	public int GetWallNumber(int x, int y){
		Debug.Log("Debut GetWallNumber");
		int num = 0;
		for(int i = -1; i<1; i++){
			Debug.Log("For 1 GetWallNumber");
			for (int j = -1; j<1; j++){
				Debug.Log("For 2 GetWallNumber");
				if (i==0&&j==0){
					continue;
				}

				if(getHexa(x+i,y+j)!=null){
					Debug.Log("if 1 GetWallNumber");
					if (getHexa(x + i, y + j).type == HexaType.WALL)
                    {
						Debug.Log("if 2 GetWallNumber");
						num++;
					}
		...
		Debug.Log("Fin GetWallNumber: "+num);
		return num;
	}
	...
	public void createRandomRectGrid2(int w,int h){
		...
		for (int j=0;j<h;j++){
			for (int i=0;i<w;i++){
				...
				if(GetWallNumber(i,j)>=4||isHexaBonus(getHexa(i,j))){
	...
	public void addChar(CharClass charClass, int x, int y, int team){
		...
		if (hexa != null && hexa.charOn == null){
			Character c = new Character(charClass , this.getRoleOfCharacter(charClass), x, y, team);
			hexa.charOn = c;
			charList.Add(c);
		}
	}
	...
	public void addChar2(CharClass charClass, int x, int y, int team){
		Hexa hexa = getHexa(x,y);
		if (hexa != null && hexa.charOn == null){
			Character c = new Character(charClass, this.getRoleOfCharacter(charClass), x, y, team, true);
			hexa.charOn = c;
			charList.Add(c);
		}
	}
